...
const AuthRoutes = () => {
  ...
  const getUserTasks = async () => {
    ...
    await taskdispatch({ type: "get_user_tasks", payload: Array.isArray(res.data) ? res.data : [] });
  };

  const getUserTeams = async () => {
    const id = localStorage.getItem("userId");
    const res = await apiServer.get(`/team/user/${id}`);
    await teamdispatch({ type: "get_user_teams", payload: Array.isArray(res.data) ? res.data : [] });
  };
